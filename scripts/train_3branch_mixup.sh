#!/usr/bin/env bash
# Train AIDE_3BRANCH with Manifold Mixup + HPF branch dropout for better cross-domain generalization.
# Usage:
#   Single GPU:  bash scripts/train_3branch_mixup.sh
#   Dual GPU:    NUM_GPUS=2 bash scripts/train_3branch_mixup.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
cd "${REPO_DIR}"

TRAIN_DATA="/data/CNNSpot/progan_train"
EVAL_DATA="/data/CNNSpot/progan_val"
RESNET_PATH="/AIGCDetect/models/123456/pretrained_ckpts/resnet50.pth"
CONVNEXT_PATH="/AIGCDetect/models/123456/pretrained_ckpts/open_clip_pytorch_model.bin"
NPR_PATH="/AIGCDetect/models/123456/pretrained_ckpts/NPR.pth"
OUTPUT_DIR="/AIGCDetect/models/123456/results/3branch_mixup"

NUM_GPUS=${NUM_GPUS:-2}

if [ "${NUM_GPUS}" -eq 1 ]; then
  BATCH_SIZE=32
  UPDATE_FREQ=8
  NUM_WORKERS=8
else
  BATCH_SIZE=32
  UPDATE_FREQ=4
  NUM_WORKERS=8
fi

BLR=1e-4
WARMUP_EPOCHS=2
EPOCHS=20
SAVE_FREQ=1
WEIGHT_DECAY=0.05
NPR_BRANCH_DROPOUT=0.3
HPF_BRANCH_DROPOUT=0.3

PY_ARGS=("$@")

for p in "${TRAIN_DATA}" "${EVAL_DATA}" "${RESNET_PATH}" "${CONVNEXT_PATH}" "${NPR_PATH}"; do
  if [ ! -e "${p}" ]; then
    echo "[ERROR] Not found: ${p}"; exit 1
  fi
done

mkdir -p "${OUTPUT_DIR}"

echo "=========================================="
echo "Training AIDE_3BRANCH (Mixup + HPF dropout)"
echo "  GPUs: ${NUM_GPUS}  Batch: ${BATCH_SIZE}  Effective: $((BATCH_SIZE * UPDATE_FREQ * NUM_GPUS))"
echo "  manifold_mixup=True  hpf_branch_dropout=${HPF_BRANCH_DROPOUT}"
echo "=========================================="

COMMON_ARGS=(
  --model AIDE_3BRANCH
  --data_path "${TRAIN_DATA}"
  --eval_data_path "${EVAL_DATA}"
  --resnet_path "${RESNET_PATH}"
  --convnext_path "${CONVNEXT_PATH}"
  --npr_path "${NPR_PATH}"
  --output_dir "${OUTPUT_DIR}"
  --batch_size "${BATCH_SIZE}"
  --update_freq "${UPDATE_FREQ}"
  --blr "${BLR}"
  --warmup_epochs "${WARMUP_EPOCHS}"
  --weight_decay "${WEIGHT_DECAY}"
  --epochs "${EPOCHS}"
  --num_workers "${NUM_WORKERS}"
  --use_amp True
  --freeze_npr False
  --skip_pretrained False
  --npr_proj_dim 128
  --npr_branch_dropout "${NPR_BRANCH_DROPOUT}"
  --hpf_branch_dropout "${HPF_BRANCH_DROPOUT}"
  --manifold_mixup True
  --manifold_mixup_alpha 0.2
  --save_ckpt True
  --save_ckpt_freq "${SAVE_FREQ}"
  "${PY_ARGS[@]}"
)

if [ "${NUM_GPUS}" -eq 1 ]; then
  python main_finetune.py "${COMMON_ARGS[@]}"
else
  torchrun --nproc_per_node="${NUM_GPUS}" --master_port=29502 main_finetune.py "${COMMON_ARGS[@]}"
fi

echo "Training finished."
echo "Output dir: ${OUTPUT_DIR}"
echo "Best checkpoint: ${OUTPUT_DIR}/checkpoint-best.pth"
